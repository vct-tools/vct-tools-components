...
(async () => {
  ...
  for (const agent of agentData.data) {
    for (const ability of agent.abilities) {
      ...
      if (fs.existsSync(path.join(__dirname, `../src/images/abilities/${agent.displayName.replace("/", "")}/`)) === false) {
        fs.mkdirSync(path.join(__dirname, `../src/images/abilities/${agent.displayName.replace("/", "")}/`
        ));
      }
      fs.writeFileSync(
        path.join(__dirname, `../src/images/abilities/${agent.displayName.replace("/", "")}/`, `${ability.displayName.replace("/", "").replaceAll(" ", "_").toLowerCase()}.png`),
        buf
      );
